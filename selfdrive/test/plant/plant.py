...
class Plant(object):
  ...
  def __init__(self, lead_relevancy=False, rate=100, speed=0.0, distance_lead=2.0):
    ...
    if not Plant.messaging_initialized:
      ...
      Plant.liveCalibration = messaging.pub_sock(context, service_list['liveCalibration'].port)    # for calibration; added by Hasnat
  ...
  def close(self):
    Plant.logcan.close()
    Plant.model.close()
    Plant.liveCalibration.close()   # added by Hasnat
  ...
  def step(self, v_lead=0.0, cruise_buttons=None, grade=0.0, frameIdx=0, publish_model = True, publish_cal = True):   # frameIdx added by Hasnat
    ...
    '''
    if frameIdx < 150 or frameIdx > 250:
      self.brake_error = 0
    else:
      self.brake_error = 1
    '''
    ...
    if (self.rk.frame%(self.rate/5)) == 0:
      print "%6.2f m  %6.2f m/s  %6.2f m/s2   %.2f ang   gas: %.2f  brake: %.2f  steer: %5.2f     lead_rel: %6.2f m  %6.2f m/s" % (distance, speed, acceleration, self.angle_steer, gas, brake, steer_torque, d_rel, v_rel) #commented by Hasnat
    ...
    for msg in set(msgs):
      ...
      #if frameIdx < 150 or frameIdx > 250:    # to check the CAN commIssue
    ...
    radar_msg = to_3_byte(d_rel*16.0) + \
                ...
                "0f00000"
    #if frameIdx < 3001: # or frameIdx > 250:   # to check the radarCommIssue
    can_msgs.append([0x445, 0, radar_msg.decode("hex"), 1])     # addind radar message to CAN -- Hasnat
    ...
    if publish_model:
      ...
      md.init('model')
      md.model.frameId = frameIdx # added by Hasnat
      #md.model.frameId = 0    # original code
      for x in [md.model.path, md.model.leftLane, md.model.rightLane]:
        x.points = [0.0]*50    # original value 0.0
        #x.points = [random.randint(0,10)/10.0]*50    #Hasnat
        '''
        if frameIdx<1500:
          x.points = [0.0]*50   
        else:
          x.points = [random.randint(0,100)/10.0]*50
        '''
        x.prob = 1.0
        x.std = 1.0
      # fake values?
      #if md.model.frameId < 500:# or md.model.frameId > 250:  # to check DataNeeded Issue
      Plant.model.send(md.to_bytes())

    # ******** calibration *********   # added by Hasnat
    if publish_cal:
      cal = messaging.new_message()
      cal.init('liveCalibration')
      cal.liveCalibration.calStatus = 1
      cal.liveCalibration.calPerc = 100
      Plant.liveCalibration.send(cal.to_bytes())
